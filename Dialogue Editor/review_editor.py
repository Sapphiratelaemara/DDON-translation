...
class ReviewEditor(SharedEditorMixin, tk.Toplevel):
    ...
    @property
    def cm(self):
        """Uniform access to ConfigManager — lets mixin methods use self.cm throughout."""
    ...
    def load_item(self):
        ...
        if self.speaker_name:
            ...
            self.archetype_combo.event_generate("<<ComboboxSelected>>")
        ...
        et_rules_now = self.cm.config.get("entry_type_rules", {}).get(self.entry_type, {})
        ...
        if self.current_category == "Tag Issues (Complex Tags)":
            ...
            if instances:
                ...
                seen_tags    = list(dict.fromkeys(unknown_tags))
    ...
    def save_item(self):
        ...
        # Blocker 3: tag mismatch vs Japanese source
        ...
        jp_counts = Counter(jp_tags)
        en_counts = Counter(en_tags)
